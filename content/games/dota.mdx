---
name: Dota 2
icon: "../img/games/dota/icon.svg"
banner: "../img/games/dota/banner.png"
logo: "../img/games/dota/logo.svg"
slug: dota
links:
  - text: Rules coming soon
    icon: book
    href: "/rules/dota.pdf"
    class: "btn-secondary"
    disabled: true
  - text: Smash.gg Page
    icon: smashgg
    href: "https://smash.gg/tournament/gamefest-2019/events/dota-2-5v5-byoc"
    class: "btn-secondary"
  - text: Register
    icon: ticket-alt
    href: "/register/"
    class: "btn-primary"
---

#### Tournament Overview

Gamefest 2019 will have a Dota 2 tournament that is scheduled to occur **only on the first day, November 2<sup>nd</sup>**, though overflow onto the second day may occur if more time is needed. The tournament details are as follows:

- 5v5, team-based
- Bring-your-own-computer
- Tournament held on first day, November 2<sup>nd</sup>

##### Competitive Structure

Each tournament round will use the **Captain's mode** format and will have an overall format consisting of group stage and playoff sections (played on days 1 and 2, respectively):

- Day 1: Team average solo MMRs will be recorded and two groups will be divided. The top four overall teams will proceed to the playoffs
- Day 2: Semifinals will be played between the finalists of each group and the runner-up of the other group

##### Placement

First and second place will be determined by a best-of-five grand finals
Third place will be decided by the winner of a best-of-three decider series
